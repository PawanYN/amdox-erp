...
docker exec $ContainerName bash -c "/opt/keycloak/bin/kcadm.sh create clients -r $Realm -s clientId=$Client -s enabled=true -s publicClient=false -s secret=amdox-secret-123 -s 'redirectUris=[\"http://localhost:3000/*\", \"http://localhost:3001/*\"]' -s standardFlowEnabled=true -s directAccessGrantsEnabled=true -s 'webOrigins=[\"+\"]'"
