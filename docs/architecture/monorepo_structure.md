...
| API modules: auth, finance, hr, scm, notification, audit, health | Also: `tenant`, `pm`, `bi`, `forecast` (business modules, at `apps/api/src/` top level) plus `graphql`, `search`, `observability`, `common`, `health` (cross-cutting, moved under `apps/api/src/infrastructure/` — see `docs/architecture/backend_architecture.md` §8) — 15 module folders total                                                                                   |
| `scripts/` — seed-db, generate-postman, migrate                  | Reorganized into `scripts/{keycloak,data-seed,scaffolding,reporting}/` (see `scripts/README.md`) — `reporting/generate-erd.mjs` (ERD from live schema); `apps/api/scripts/audit-tenant-scoping.ts` (CI security gate), `verify-payroll-retry.ts`, plus 4 backfill scripts + a seeder, all wired into `apps/api/package.json` (see `apps/api/scripts/README.md`)                    |
| Notification email via AWS SES                                   | Nodemailer → Mailpit (dev) / SMTP (prod); SES is a config swap-in                                                                                                                                                                                                                                                                                                                  |
| `docs/` — flat collection of ~35 files                           | Reorganized into `docs/{architecture,audits,planning,guides,reference}/` by topic, plus the pre-existing `adr/`, `api/`, `c4/`, `erd/`, `learning/`, `fixtures/`, `doubts/` (see `docs/README.md`)                                                                                                                                                                                 |
