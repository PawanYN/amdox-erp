...
**Completion:** ~86% done · ~2% partial · ~12% not started
...
• Postman collection auto-generated from OpenAPI spec ⚠️ Partial — `openapi-spec.json` auto-generates ✅, but the Postman file (`docs/Amdox-ERP-HR-Postman-Collection.json`) is a manually-scoped HR collection, not a full auto-conversion
...
• Multi-currency support: ECB/OpenExchangeRates daily FX fetch ⚠️ (`fx-rate.service.ts` — mocked cron, not a real external API call) + historical rates ⚠️
• AP module: invoice creation ✅, 3-way PO/GR/Invoice matching ✅ (`invoice-matching.service.ts`), payment run logic ❌ (approve-only, no batch payment run)
...
• Organisational chart (recursive CTE query in Postgres) ❌ (hierarchy exists via self-relation, no recursive CTE endpoint)
...
• Database query optimisation (EXPLAIN ANALYZE on critical queries) ❌
...
• Recharts ✅ + ECharts ❌ React components: bar ✅, line ✅, pie ✅, heatmap ✅, funnel ✅ (types supported per `Widget.type`, ECharts library not used)
• Drill-down: click chart segment → filtered data table ✅ (`components/bi/drill-down-panel.tsx`)
• Scheduled report job: generate PDF ✅ /Excel ⚠️ (CSV fallback, not true `.xlsx` — `bi-report.service.ts`) + email delivery ❌ (stub)
...
• Identify bottlenecks: N+1 queries, missing indexes, Redis cache gaps ❌
...
> None of this day's items were found in the repo.

### Week 4 – Deployment, Observability & Final Polish

#### Day 22 – Containerisation

• Multi-stage Dockerfiles for all services (Distroless final stage) ❌ (`infra/docker/Dockerfile.api` and `Dockerfile.web` are 0-byte empty files; only `apps/ml-service/Dockerfile` exists, single-stage `python:3.11-slim`, not distroless)
