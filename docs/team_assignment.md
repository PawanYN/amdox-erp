...
• Notification service: in-app (SSE / Socket.io) ❌ (stored + polled via `/notifications`, not push — no in-app push mechanism exists anywhere), email (SES) ❌ (no email path in the notification engine at all — `notify()` only ever dispatches IN_APP + WEBHOOK; a separate log-only stub exists but is only used by BI scheduled reports), webhook (signed HMAC) ✅ (`notification/channels/webhook.channel.ts` — HMAC-SHA256, real dispatch from `notify()`)
