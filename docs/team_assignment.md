...
• Notification service: in-app (SSE / Socket.io) ❌ (stored + polled via `/notifications`, not push — no in-app push mechanism exists anywhere), email ⚠️ (real dispatch path now wired into `notify()` — looks up the target user's email, respects the `EMAIL` `NotificationPreference`, calls `EmailChannel.send()`, records a `NotificationDelivery` row; verified live with a real `employee.created` event — `EMAIL` delivery recorded `SENT`, and disabling the `EMAIL` preference correctly suppressed only that channel while `IN_APP` still fired. Only real gap left: `EmailChannel` itself has no AWS SES credentials (BE-07) so it logs instead of actually sending over the wire — the moment those credentials are added, this path starts delivering for real with no further code changes), webhook (signed HMAC) ✅ (`notification/channels/webhook.channel.ts` — HMAC-SHA256, real dispatch from `notify()`)
...
• BullMQ dead-letter queue + retry dashboard (Bull Board UI) ❌ (neither found — `notification.processor.ts` is a dead empty stub, not registered as a provider in `notification.module.ts`; no `BullModule.registerQueue` anywhere in the module)
