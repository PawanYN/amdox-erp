...
const KC_BASE = typeof window !== "undefined"
...
function GoogleForm({
  ...
  const submit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      displayName: "Google",
      enabled: true,
      config: { clientId, clientSecret, hostedDomain, useUserIpParam: String(useUserIp), requestRefreshToken: String(requestRefresh), guiOrder: displayOrder },
    });
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-0">
      ...
      <FormField label="Client ID" required>
        <input value={clientId} onChange={e => setClientId(e.target.value)} required placeholder="Your Google OAuth Client ID" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none" />
      </FormField>
      <FormField label="Client Secret" required>
        <SecretInput value={clientSecret} onChange={setClientSecret} />
      </FormField>
      <FormField label="Display order" hint="Order in which this provider appears on the login page.">
        <input value={displayOrder} onChange={e => setDisplayOrder(e.target.value)} type="number" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Hosted Domain" hint="Set the 'hd' query parameter when logging in with Google. Google will list accounts only for this domain. When '*' is entered, any hosted account can be used. Comma ',' separated list of domains is supported.">
        <input value={hostedDomain} onChange={e => setHostedDomain(e.target.value)} placeholder="yourcompany.com" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
...
function MicrosoftForm({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ alias: "microsoft", providerId: "microsoft", displayName: "Microsoft", enabled: true, config: { clientId, clientSecret, guiOrder: displayOrder, tenantId } });
  };
  ...
  return (
    <form onSubmit={submit}>
      ...
      <FormField label="Client ID" required>
        <input value={clientId} onChange={e => setClientId(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Client Secret" required>
        <SecretInput value={clientSecret} onChange={setClientSecret} />
      </FormField>
      ...
      <FormField label="Tenant ID" hint="Uses single-tenant auth endpoints when specified, uses 'common' multi-tenant endpoints otherwise.">
        <input value={tenantId} onChange={e => setTenantId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormActions loading={loading} onCancel={onCancel} />
    </form>
  );
...
function GitHubForm({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ alias: "github", providerId: "github", displayName: "GitHub", enabled: true, config: { clientId, clientSecret, guiOrder: displayOrder, baseUrl, apiUrl } });
  };
  ...
  return (
    <form onSubmit={submit}>
      ...
      <FormField label="Base URL" hint="Override the default Base URL for this identity provider.">
        <input value={baseUrl} onChange={e => setBaseUrl(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="API URL" hint="Override the default API URL for this identity provider.">
        <input value={apiUrl} onChange={e => setApiUrl(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormActions loading={loading} onCancel={onCancel} />
    </form>
  );
...
function SamlForm({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ alias, providerId: "saml", displayName, enabled: true, config: { guiOrder: displayOrder, entityId, useMetadataDescriptorUrl: String(useDescriptor), metadataDescriptorUrl: entityDescriptor } });
  };
  ...
  return (
    <form onSubmit={submit}>
      ...
      <FormField label="Alias" required>
        <input value={alias} onChange={e => setAlias(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Display name">
        <input value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Display order">
        <input value={displayOrder} onChange={e => setDisplayOrder(e.target.value)} type="number" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      ...
      <FormField label="Service provider entity ID" required>
        <input value={entityId} onChange={e => setEntityId(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Use entity descriptor" hint="When enabled, import SAML settings automatically from the IDP's metadata URL or XML.">
        <div className="flex items-center gap-2.5 pt-0.5">
      ...
      {useDescriptor && (
        <FormField label="SAML entity descriptor" required hint="URL or XML of the SAML Identity Provider metadata.">
          <input value={entityDescriptor} onChange={e => setEntityDescriptor(e.target.value)} required placeholder="https://idp.example.com/saml/metadata" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
        </FormField>
      )}

      <div className="py-2">
        <button type="button" onClick={() => setShowMeta(s => !s)} className="flex items-center gap-1.5 text-xs text-blue-600 hover:underline">
          <ChevronRight size={13} className={`transition-transform ${showMeta ? "rotate-90" : ""}`} />
          Show metadata
        </button>
        {showMeta && (
          <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-md text-[11px] text-slate-600 space-y-1">
            <p>SP Metadata URL: <span className="font-mono text-blue-600">{KC_BASE}/realms/{tenantSlug}/broker/{alias || "saml"}/endpoint/descriptor</span></p>
          </div>
        )}
      </div>
...
function OidcForm({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({
      alias, providerId, displayName, enabled: true,
      config: { guiOrder: displayOrder, useDiscoveryEndpoint: String(useDiscovery), discoveryEndpoint, clientAuthMethod: clientAuth, clientId, clientSecret, clientAssertionSignatureAlgorithm: sigAlgo },
    });
  };

  return (
    <form onSubmit={submit}>
      <RedirectUriField providerId={providerId} alias={alias || defaultAlias} tenantSlug={tenantSlug} />
      <FormField label="Alias" required>
        <input value={alias} onChange={e => setAlias(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
      </FormField>
      <FormField label="Display name">
      ...
      <FormField label="Use discovery endpoint" hint="Automatically fetch OpenID Connect configuration from a discovery URL.">
        <div className="flex items-center gap-2.5 pt-0.5">
      ...
      {useDiscovery && (
        <FormField label="Discovery endpoint" required hint="The well-known OpenID configuration URL.">
          <input value={discoveryEndpoint} onChange={e => setDiscoveryEndpoint(e.target.value)} required placeholder="https://hostname/auth/realms/master/.well-known/openid-configuration" className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
        </FormField>
      )}
      ...
      <div className="py-2">
        ...
        {showMeta && (
          <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-md text-[11px] text-slate-600">
            <p>Issuer, auth endpoint, and token endpoint will be auto-populated from the discovery document.</p>
          </div>
        )}
      </div>

      <FormField label="Client authentication">
        <select value={clientAuth} onChange={e => setClientAuth(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs bg-white focus:ring-1 focus:ring-blue-500 outline-none">
      ...
      <FormField label="Client assertion signature algorithm">
        <select value={sigAlgo} onChange={e => setSigAlgo(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-xs bg-white focus:ring-1 focus:ring-blue-500 outline-none">
...
function GenericSocialForm({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ alias: provider.id, providerId: provider.id, displayName: provider.label, enabled: true, config: { clientId, clientSecret, guiOrder: displayOrder } });
  };
...
export function IdpManager({ identityProviders, tenantSlug, onAdd, onDelete }: IdpManagerProps) {
  ...
  const handleCancel = () => {
    if (view === "form") { setView("picker"); return; }
    setView("list");
    setSelectedProvider(null);
  };
  ...
  const breadcrumb = (
    <div className="flex items-center gap-1.5 text-xs mb-5">
      <button type="button" onClick={() => setView("list")} className="text-blue-600 hover:underline font-medium">
        Identity providers
      </button>
      {(view === "picker" || view === "form") && (
        <>
          <ChevronRight size={12} className="text-slate-400" />
          <button type="button" onClick={() => setView("picker")} className={view === "picker" ? "text-slate-700 font-medium" : "text-blue-600 hover:underline font-medium"}>
            Add provider
          </button>
        </>
      )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${idp.enabled !== false ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-200"}`}>
      ...
      <div>
        ...
        <div className="mb-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">User-defined</span>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">Social</span>
            <div className="flex-1 h-px bg-slate-200" />
          </div>
    ...
    return (
      <div>
        ...
        {selectedProvider.id === "google" && (
          <GoogleForm tenantSlug={tenantSlug} onSubmit={handleSubmit} onCancel={handleCancel} loading={loading} />
        )}
        {selectedProvider.id === "microsoft" && (
          <MicrosoftForm tenantSlug={tenantSlug} onSubmit={handleSubmit} onCancel={handleCancel} loading={loading} />
        )}
        {selectedProvider.id === "github" && (
          <GitHubForm tenantSlug={tenantSlug} onSubmit={handleSubmit} onCancel={handleCancel} loading={loading} />
        )}
        {selectedProvider.id === "saml" && (
          <SamlForm tenantSlug={tenantSlug} onSubmit={handleSubmit} onCancel={handleCancel} loading={loading} />
        )}
        ...
        {!["google","microsoft","github","saml","keycloak-oidc","oidc"].includes(selectedProvider.id) && (
          <GenericSocialForm provider={selectedProvider} tenantSlug={tenantSlug} onSubmit={handleSubmit} onCancel={handleCancel} loading={loading} />
        )}
      </div>
    );
  }
