...
export default function DashboardHome() {
  ...
  if (token) {
    try {
      ...
      if (roles.includes("tenant_admin") || roles.includes("TENANT_ADMIN")) { role = "Tenant Admin"; isAdmin = true; }
  ...
  useEffect(() => {
    ...
    (async () => {
      let prof: any = null;
      try { prof = await hrApi.getMe(); setProfile(prof); } catch {}
      if (!prof?.id) return;
      try {
  ...
  const handleClockIn = async () => {
    ...
    try { await hrApi.clockIn(profile.id); setClockedIn(true); setTodaysClockIn(new Date()); }
    catch (e: any) { alert(e.message || "Failed to clock in"); }
  };

  const handleClockOut = async () => {
    if (!profile?.id) return;
    try { await hrApi.clockOut(profile.id); setClockedIn(false); setTodaysClockIn(null); }
    catch (e: any) { alert(e.message || "Failed to clock out"); }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-card p-5 flex items-center gap-4">
        <div className="h-14 w-14 rounded-full bg-blue-600 flex items-center justify-center text-white text-lg font-semibold shrink-0">
          {(profile?.fullName || name).split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase()}
        </div>
        <div className="min-w-0">
          <h1 className="text-lg font-semibold text-slate-900 truncate">{profile?.fullName || name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-card p-5">
            <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-wider mb-4">Profile Details</p>
          ...
          {!isAdmin && (
            <div className={`rounded-lg border p-5 transition-all duration-300 ${
              clockedIn ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white shadow-card"
            }`}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className={`text-[13px] font-semibold ${clockedIn ? "text-emerald-800" : "text-slate-900"}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white rounded-lg border border-slate-200 shadow-card p-5">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Annual Leave</p>
                </div>
              ...
              <div className="bg-white rounded-lg border border-slate-200 shadow-card p-5">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Sick Leave</p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg border border-slate-200 p-4 space-y-3 animate-fade-in-up">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      <label className="block text-[11px] text-slate-500 font-medium uppercase mb-1">Leave Type</label>
                      <select className="input-base" value={leaveType} onChange={(e) => setLeaveType(e.target.value)}>
                        {balances.map((b) => (
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-500 font-medium uppercase mb-1">Start Date</label>
                      <input type="date" className="input-base" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-[11px] text-slate-500 font-medium uppercase mb-1">End Date</label>
                      <input type="date" className="input-base" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-[11px] text-slate-500 font-medium uppercase mb-1">Reason (optional)</label>
                      <input type="text" placeholder="Brief reason…" className="input-base" value={reason} onChange={(e) => setReason(e.target.value)} />
                    </div>
                  </div>
                  <button
                    onClick={submitLeave}
                ...
                <div className="space-y-2">
                  {requests.length === 0 ? (
                    ...
                    requests.slice(0, 5).map((req) => (
                      <div key={req.id} className="flex items-center justify-between py-2.5 px-3 rounded-md border border-slate-100 hover:border-slate-200 hover:bg-slate-50/50 transition-colors">
                        <div>
                          <p className="text-[13px] font-semibold text-slate-800">{req.leaveType?.name || "Leave"}</p>
