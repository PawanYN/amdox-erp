...
function BudgetCard({ b }: { b: Budget }) {
  ...
  return (
    <div
      className={`border rounded-lg bg-white overflow-hidden shadow-card ${b.isOverrun ? "border-red-200" : "border-slate-200"}`}
    >
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <span
            className={`text-[12px] font-mono font-semibold w-10 text-right ${b.isOverrun ? "text-red-600" : "text-slate-600"}`}
          >
            {pct}%
          </span>
        </div>
...
export default function ProjectsBudgetPage() {
  ...
  if (loading) return <p className="text-sm text-slate-500">Loading budgets…</p>;

  return (
    <div className="space-y-3">
      {budgets.length === 0 ? (
        <p className="text-sm text-slate-500">No project budgets set yet.</p>
      ) : (
