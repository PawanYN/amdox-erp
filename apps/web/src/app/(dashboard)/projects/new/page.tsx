    ...
    <div className="space-y-5">
      <Field label="Project name" required>
        <input
          ...
          placeholder="e.g. Warehouse Expansion — Pune"
          value={data.name}
      ...
      <Field
        label="Scope"
        required
        hint="What this project covers — used to set boundaries with PM coordination"
      >
        <textarea
          className={inputClass}
          rows={3}
...
function StepTasks({
  ...
  const addMilestone = () => {
    ...
    setMilestones([
      ...milestones,
      { id: `M-${milestones.length + 1}`, name: newMilestone.name, dueDate: newMilestone.dueDate },
    ]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 py-2 text-[13px] font-medium rounded-md bg-emerald-600 text-white whitespace-nowrap hover:bg-emerald-700 transition-colors"
          >
            + Add milestone
          </button>
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          <button
            onClick={addTask}
            className="px-3 py-2 text-[13px] font-medium rounded-md bg-blue-600 text-white whitespace-nowrap hover:bg-blue-700 transition-colors"
          >
            + Add task
          </button>
        </div>
        ...
        {hasCycle(tasks) && (
          <div className="flex items-center gap-2 text-[12px] text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2 mt-2">
            <AlertTriangle size={14} /> Circular dependency detected — a task cannot depend on
            itself.
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-500">
        Listens for <span className="font-mono text-blue-700">project.resource_needed</span> from
        HR. Assigning here maps{" "}
        <span className="font-mono text-blue-700">Employee.id ↔ Task.id</span> — PM never writes to
        HR&apos;s Employee table directly.
      </div>
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      for (const t of tasks) {
        const dependsOn =
          t.dependsOn && taskIdMap[t.dependsOn] ? [taskIdMap[t.dependsOn]] : undefined;
        const milestoneId =
          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                <div
                  className={`h-8 w-8 rounded-full flex items-center justify-center border-2 text-[12px] font-semibold transition-colors ${
                    i < step
              ...
              {i !== STEPS.length - 1 && (
                <div
                  className={`flex-1 h-[2px] mx-2 mb-4 transition-colors ${i < step ? "bg-blue-600" : "bg-slate-200"}`}
                />
              )}
            </div>
          ))}
